...
export default function Search() {
        ...
        const [rideClickCounts, setRideClickCounts] = useState<{ [key: string]: number }>({});


    const handleRideClick = (data: any) => {
            setSelectedRide(data);
            setRideClickCounts((prevCounts) => {
                const newCount = (prevCounts[data.title] || 0) + 1;

                if (newCount === 2) {
                    setModalData(data);
                }

                return { ...prevCounts, [data.title]: newCount };
            });
        };
            ...
            <div className={styles.searchMainDiv}>
                {!showRideDetails ? (
                    ...
                    <div className="w-full ">
                        ...
                        <div className="overflow-y-auto">
                            {RideDetailsOptions.map((ride, index) => (
                                <RideDetails
                                    ...
                                    time={ride.time}
                                    text={ride.text}
                                    price={`₦${ride.baseFare + fareInNaira.toFixed(2)}`}
                                    classname={ride.classname}
                                    onClick={() => handleRideClick(ride)}
                                    isActiveBorder={selectedRide?.title === ride.title}
                                />
                            ))}
                        </div>
                        ...
                        {/* Payment Section */}
                        <div
                            className="bg-fixed shadow-md bottom-0 left-0 right-0 z-10 flex justify-between items-center gap-4 border rounded-lg p-3 bg-white mt-4">
                            ...
                            <button className="rounded bg-black text-white px-4 py-2 text-sm">
                                Request {selectedRide ? `${selectedRide.title}` : ""}
                            </button>
                        </div>
                        ...
                        {modalData && (
                            <RideDetailsModal
                                ...
                                // minimumFare={selectedRide}
                                perKilometerFare={FareCharges[0].pricePerKilometer}
                                perMinuteFare={FareCharges[0].pricePerMinute}
                            />
                        )}
                    </div>
                )}
